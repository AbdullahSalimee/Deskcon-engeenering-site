...
<div class="relative w-full overflow-hidden h-[600px]">
    ...
    <div class="absolute inset-0 bg-primary-900/60 flex items-center z-10">
        <div class="mx-auto text-center text-primary-500">
            <div class="text-7xl font-extralight brightness-200">"Engineering Progress"</div>
            <div class="text-primary-300 font-bold text-5xl mt-2">"Designing the Future"</div>
    ...
    <button
        on:click={prevSlide}
        class="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-black/40 text-white px-4 py-2 text-2xl rounded-full hover:bg-black/70"
        >‹</button
    >
    <button
        on:click={nextSlide}
        class="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-black/40 text-white px-4 py-2 text-2xl rounded-full hover:bg-black/70"
        >›</button
    >
</div>