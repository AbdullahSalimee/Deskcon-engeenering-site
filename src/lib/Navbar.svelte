...
onMount(() => {
    ...
    const scrollOffset = 150; // Adjust this value to control how much higher to scroll (in pixels)

    // Calculate and cache section positions
    ...
    function updateActiveSection() {
        ...
        sectionIds.forEach(id => {
            ...
            if (section) {
                ...
                const visibleBottom = Math.max(0, section.bottom - (scrollPosition + window.innerHeight * 2/3));
    ...
    // Function to handle programmatic scrolling (when a nav item is clicked)
    function scrollToSection(id) {
        const section = sectionPositions.get(id);
        if (section) {
            window.scrollTo({
                top: section.top - scrollOffset, // Scroll to position above the section
                behavior: 'smooth'
            });
        }
    }
    ...
    // Export the scrollToSection function to be used by navigation items
    return {
